...
class LlavaForConditionalGeneration(LlavaPreTrainedModel):
    ...
    def _merge_input_ids_with_image_features(self, image_features, inputs_embeds, input_ids, attention_mask, labels):
        ...
        ###OURS###
        per_img_token = image_features.shape[1]
        image_token_indices = torch.nonzero(image_to_overwrite, as_tuple=False)[:, 1].tolist()
        img_str_idx = image_token_indices[0::per_img_token]
        img_str_idx.append(per_img_token)
        # print("img_str_idx: ",img_str_idx)
        ###OURS###
        final_embedding[image_to_overwrite] = image_features.contiguous().reshape(-1, embed_dim).to(target_device)  #reshape即所有图像 token 的嵌入串成一条大长条
        ...
        return final_embedding, final_attention_mask, final_labels, position_ids, img_str_idx
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                if not self.training:
                    ...
                    all_image_features = []
                    # print("pixel_values.shape: ",pixel_values.shape)  #代表几张图片
                    for i in range(0, pixel_values.shape[0], pixel_batch_size):
                        ...
                        if vision_feature_select_strategy == "default":
                            # 移除 [CLS] 或类似 token，仅保留 patch-level 特征。
                        ...
                        batch_image_features = self.multi_modal_projector(batch_selected_image_features)    #将视觉转换到语言空间
                        # print("batch_image_features: ",batch_image_features.shape)
                        all_image_features.append(batch_image_features)
                    # print("all_image_features: ",len(all_image_features))
                ...
                # print('before input_emb: ', inputs_embeds.shape)
                # print('input_ids: ',input_ids.shape)
                before_input_emb = inputs_embeds
                inputs_embeds, attention_mask, labels, position_ids, img_str_idx = self._merge_input_ids_with_image_features(
                    image_features, inputs_embeds, input_ids, attention_mask, labels
                )
                after_input_emb = inputs_embeds
                # print('after input_emb: ', inputs_embeds.shape)

                self.img_str_idx = img_str_idx
                if labels is None:
        ...
        # print("self.img_str_idx: ",self.img_str_idx)
        ...
        model_inputs.update(
            {
                ...
                "base_ratio": kwargs.get("base_ratio"),
            }
        )
        return model_inputs
