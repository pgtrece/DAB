...
def reallocation_img_attn_ratio_avg(img_str_idx, attention_map, base_ratio=0.2, alpha=0.5):
    ...
    '''
    For the attention heads with a visual attention ratio greater than base_ratio and the text tokens,
    calculate the average attention for all images.
    Then, for images with greater attention, subtract the interpolation/per_img_token_num for each token.
    For images with smaller attention, add the interpolation/per_img_token_num for each token.
    '''
    ...
    # Step 1: Calculate the total attention for all image areas at once
    img_sums = []
    for i in range(n):
    ...
    # Step 2: Calculate the total ratio and average attention
    # total_ratio: The ratio for all images = the ratio of total visual tokens
    total_ratio = img_sums.sum(dim=0)  # [num_heads, num_text_token]
    avg_per_img = total_ratio / n  # [num_heads, num_text_token]

    # Step 3: Calculate the deviations and scaling factors for all images
    ...
    # Step 4: Apply the mask to determine the positions that need to be updated
    ...
    # Step 5: Directly update the attention map to avoid creating intermediate tensors
    for i in range(n):
